...
class Livraison:
    def __init__(self, id=None, livreur_id=None, commande_id=None, attribue_le=None):
        self.id = id
        self.livreur_id = livreur_id
        self.commande_id = commande_id
        self.attribue_le = attribue_le
    ...
    @staticmethod
    def update(livraison_id, statut_id):
        if not isinstance(livraison_id, int) or livraison_id <= 0 or not isinstance(statut_id, int) or statut_id <= 0:
            return {"error": "ID invalide"}
