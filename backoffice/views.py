...
def restaurants_list(request):
    secteur = request.GET.get('secteur')  # nom de zone
    horaire_str = request.GET.get('horaire')  # 'HH:MM' format attendu
    ...
    if secteur:
        restaurants = restaurants.filter(
            zonerestaurant__zone__nom__iexact=secteur
        )

    if horaire_str:
        restaurants = restaurants.filter(
            horaire__le_jour=current_day,
            horaire__horaire_debut__lte=horaire_str,
            horaire__horaire_fin__gte=horaire_str
        )

    restaurants = restaurants.distinct()

    secteurs = Zone.objects.values_list('nom', flat=True).distinct()

    # Horaires du jour
    horaires_du_jour = {
        r.id: list(r.horaire.filter(le_jour=current_day)) for r in restaurants
    }

    # Dernier statut par restaurant
    dernier_statuts = {}
    for r in restaurants:
        hs = HistoriqueStatutRestaurant.objects.filter(restaurant=r).order_by('-mis_a_jour_le').first()
        dernier_statuts[r.id] = hs.statut.appellation if hs else None

    # Dernière commission par restaurant
    dernier_commissions = {}
    for r in restaurants:
        c = Commission.objects.filter(restaurant=r).order_by('-mis_a_jour_le').first()
        dernier_commissions[r.id] = c.valeur if c else None

    # Zones par restaurant
    zones_par_restaurant = {}
    for r in restaurants:
        zones = ZoneRestaurant.objects.filter(restaurant=r).values_list('zone__nom', flat=True)
        zones_par_restaurant[r.id] = list(zones)

    # Préparer la liste finale pour le template
    restaurants_data = []
    for r in restaurants:
        restaurants_data.append({
            'restaurant': r,
            'horaires': horaires_du_jour.get(r.id, []),
            'statut': dernier_statuts.get(r.id),
            'commission': dernier_commissions.get(r.id),
            'zones': zones_par_restaurant.get(r.id, []),
        })
